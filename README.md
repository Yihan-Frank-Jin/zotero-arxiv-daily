...
If the arXiv metadata API remains rate-limited (HTTP 429), returns a server error, or cannot be reached after bounded retries, retrieval uses the current Atom feed for the remaining papers. Titles, authors, abstracts, and download links are preserved; full-text processing continues normally. An invalid feed or incomplete fallback metadata still produces an error instead of silently reporting no papers.
